...
def get_tonechart_data(keyword):
    data = requests.get("https://api.gdeltproject.org/api/v2/doc/doc?&FORMAT=json",
                        params={"query": str(keyword), "mode": "tonechart"}).json()
...
def get_geo_url(keyword):
    geo_data = requests.get(
        "https://api.gdeltproject.org/api/v2/geo/geo?", params={"query": str(keyword)})
    return geo_data.url


def get_line_data(keyword):
    line_data = requests.get("https://api.gdeltproject.org/api/v2/doc/doc?&FORMAT=json",
                             params={"query": str(keyword), "mode": "timelinevolraw",
                                     "TIMELINESMOOTH": 5}).json()
    date_labels = []
